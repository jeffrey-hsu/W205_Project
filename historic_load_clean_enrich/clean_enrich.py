...
# Filter to only include data after 1993
financial_suite = financial_suite.filter(financial_suite.qdate.cast("INT") > 19930000)
crsp = crsp.filter(crsp.datadate.cast("INT") > 19930000)
beta = beta.filter(beta.DATE.cast("INT") > 19930000)

# Convert datframes to Pandas
df = financial_data.toPandas()
CRSP_comp_merge = crsp.toPandas()
link_table = link.toPandas()
beta_suite = beta.toPandas()
recommendations = recs.toPandas()

# EXPORT TO PARQUET
financial_suite.write.parquet("hdfs:///user/w205/financial_data/parquet_files/fin_suite_filter")
crsp.write.parquet("hdfs:///user/w205/financial_data/parquet_files/crsp_filter")
link.write.parquet("hdfs:///user/w205/financial_data/parquet_files/link_table")
beta.write.parquet("hdfs:///user/w205/financial_data/parquet_files/beta_filter")
